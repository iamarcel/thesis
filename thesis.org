...
*** Detecting 2D Poses with OpenPose
    ...
    [[file:./img/sanity-check-openpose.jpg]]
...
*** Lifting Poses to 3D
    ...
    point. Some modifications had to be made in order to use it in this
    pipeline, though.
    ...
    size in an image, which would make the data collection step difficult.

    While the ideal solution, when using OpenPose's 2D results, would be to
    ...
    it, which would take too much time. Instead, a rough direct conversion was
    made. Before passing the 2D detections as input to 3D Pose Baseline, their
    points were reordered and the following points were added:
    ...
    The second modification was necessary to use this project for predictions.
    While the authors' code allowed running the training and validation steps,
    there was no code present to run the inference step, i.e., predicting 3D
    poses for new 2D detections. Additionally, the 2D predictions were smoothed
    before feeding them into this model, since OpenPose processes videos
    frame-by-frame resulting in motion that was not always smooth.
    
    After these two steps the 3D Pose Baseline code was usable but it did not
    yet produce results that were of adequate quality. Sometimes poses were
    completely incorrect or deformed throughout a clip. Three issues were found
    with the data.

    First, a missing point in the 2D detection had large effects on the results
    in 3D. In cases where there were only joints detected of the upper body, the
    3D detection became useless. Second, since the data is captured from
    multiple people, their size and body shape differed. Finally, the people in
    the 3D space were oriented in different directions.
