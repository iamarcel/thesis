...
*** Time series clustering
    :LOGBOOK:
    CLOCK: [2018-07-20 vr 17:19]--[2018-07-20 vr 20:19] =>  3:00
    :END:
    ...
    *Dynamic Time Warping (DTW) distance.* To calculate the DTW distance between
    two sequences $a_i, i \in {1, \ldots n}$ and $b_j, j \in {1, \ldots m}$, the
    following steps are taken:

    1. Calculate the pairwise Euclidian distance between every pair of points
       $(a_i, b_j)$ and store it in a matric $M_{ij} = d(a_i, b_j)$, where $d:
       \mathcal{R} \times \mathcal{R} \rightarrow \mathcal{R}$ is the Euclidean
       distance function.
    2. Find the shortest path from $M_{0,0}$ to $M{n,m}$, where the total weight
       of the path is the sum of the elements on this path.
    ...
    *Truangular Global Alignment Kernel (GAK) distance.* GAK methods interpret
    ...
    used in Support Vector Machines (SVM). With a GAK, it is relatively simple
    to add a penalty to certain paths. In particular, the Triangular GAK with
    parameter $T$ weights elements of the alignment by their distance to the
    ...
    cite:Cuturi:2011:FGA:3104482.3104599.
