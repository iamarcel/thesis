...
** Predicting Poses
   ...
   sequence into a thought vector. In one case, the encoder uses an RNN and a
...
*** The Encoder Network
    ...
    #+BEGIN_SRC latex :exports results :results output
      \begin{figure}
        ...
        \begin{tikzpicture}[semithick, font=\headingfont]
          ...
          \path [->, thick]
            (sentence) edge node[right] {Sentence} (splitter)
            ...
            (rnn) edge node[right] {Thought Vector} (thought)
            (rnn) edge [loop right] ()
          ;
    ...
    \paragraph{RNN-based encoder} The RNN-based encoder processes the input
    text in four steps:
    ...
    #+BEGIN_SRC latex :exports results :results output
      \begin{figure}
        ...
        \begin{tikzpicture}[semithick, font=\headingfont]
          ...
          \path [->, thick]
            (thought) edge node[right] {Thought Vector} (dropout)
            (dropout) edge (dense1)
            (dense1) edge (dense2)
            (dense2) edge (logits)
          ;
        ...
        \end{tikzpicture}
        \caption{\label{fig:class-decoder} The decoder network that decodes a thought
          vector into a probability for each class.}
      \end{figure}
    #+END_SRC
