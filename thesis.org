...
#+BEGIN_SRC elisp :exports none :session python-env
...
*** TODO Add a mask dimension to the data
*** TODO Stop predicting when mask says so
*** TODO Eigen embedding
...
**** DONE Encode die sequentie
     CLOSED: [2018-06-21 do 08:24]
**** TODO Decode + geef tussen-states mee aan decoder
...
*** Text Embedding
    ...
    vector) which is non-sequential and often is of much smaller dimensionality
    than the original data. This encoder part can then be re-used as the first
    step in a supervised problem.

    In Natural Language Processing (NLP), this method has been applied widely
    with the word2vec algorithm

    It is reasonable to assume that sentences with a similar meaning would
    correspond with similar gestures. Thus, working from a semantic
    representation rather than the text being processed directly could help in
    producing better gestures. This is precisely what text embedding systems do:
    they "interpret" text and return a representation of its semantics. For
    example, the embedded vectors of similar words or sentences will be close
    together.

    These systems are in use in various Natural Language Processing (NLP) tasks
    such as sentiment analysis and categorization
...
** The Dataset label:sec-research-dataset
   ...
   To build a system that can generate gestures for a given text using machine
   learning, one needs training data. Specifically, to predict gestures from
   text, the dataset should contain pairs of text being spoken and the
   corresponding gesture.

   Gestures can generally be represented by /poses/, the position of a person's
   joints. This can easily be used with virtual avatars but real robots require
   other types of input. The NAOqi software, for example, can only move the
   wrists to a specific position using inverse kinematics cite:TODO. For more
   precise control, the angles of its joints are expected.

   The biggest datasets available for human motion are the H36M cite:TODO and
   Panopticon cite:TODO datasets. However, both of these datasets are more
   geared towards activity recognition or pose estimation and thus include data
   for a large range of activities but not much data about people talking.
   Additionally, the text of what is being said is not available. With that in
   mind, the dataset will need to be created.

*** YouTube

    YouTube is one of the most popular websites and contains an abundance of
    video, from a wide variety of people in all kinds of environments and
    performing many activities cite:TODO. Videos with English spoken text are
    automatically transcribed which means that the subtitles for many videos are
    available, too.

    If these videos could be processed in order to extract the poses of people
    talking, a dataset could be constructed this way.
...
*** Pose Estimation
    ...
    Quite successful methods have been found to do pose estimation. The Stacked
    Hourglass cite:newell16_stack_hourg_networ_human_pose_estim, OpenPose
    ...
    2D, then "lifts" this into three dimensions, or one directly estimated the
    3D poses from an image.

    It is possible to estimate 3D poses straight from monocular images cite:TODO
    but these projects' source code is not available or they do not produce
    results as good as the other method: lifting 2D poses to 3D. The
    "3d-pose-baseline" project is what the authors consider to be a baseline for
    2D-to-3D lifting of poses; it is a simple neural network but works very
    well. The code is available on GitHub.

** Time Series Clustering label:sec-research-clustering
   :PROPERTIES:
   :Effort:   0:30
   :END:
   Instead of directly predicting poses, the problem of gesture synthesis is
   ...
   algorithm can be used to find clusters of (subsequences of) pose information.
   ...
   challenges. In order to be able to identify these motion primitives, the
   algorithm needs to be able to look at small parts of these sequences and find
   similar-looking subsequences in other pose animations. One can compare this
   to anomaly detection, albeit with more labels than thw two of "normal" and
   "exceptional".
   ...
   between different activities (e.g., walking versus sitting) is much less
   subtle than different hand gestures. The source code of this project was not
   accessible either so we have not tried this method on our data.

   To perform clustering on /whole/ time series, there are multiple methods. The
   classic Dynamic Time Warping (DTW) distance metric can be used to compare
   time series of different lengths and this can be used as a basis for a
   clustering algorithm: clustering in its general form comes down to grouping
   samples so that the samples within a group are close to each other while
   samples between groups are far away from each other, i.e., the distance
   metric is the most important component of a clustering algorithm
   cite:zolhavarieh14_review_subseq_time_series_clust. The ~dtwclust~ R package
   cite:r-dtwclust provides a DTW-based clustering algorithm with the option to
   use experiment different metrics like TADPole clustering.

** Conclusion

   This chapter provided an overview of the works that this thesis will use and
   build upon. Previous work on gesture synthesis were examined in both robots
   and virtual agents; then a selected set of recent developments in machine
   learning were described, particularly in terms of language processing and
   time-series data processing; in autoref:sec-research-dataset the available
   data was examined and the tools to create a dataset for this problem were
   outlined; finally, in autoref:sec-research-clustering time-series clustering
   algorithms were explored that could simplify this problem by discretizing it
   into a set of motion primitives.

   With these components and starting points in place, an image begins to form
   of what this complete system could look like. In the next section, these
   components will be combined in a pipeline that performs gesture synthesis.
...
* A Modern Approach to Gesture Synthesis label:sec:method
  ...
  will be done by first generating a dataset of clips ((subtitle, pose)-pairs)
...
** Development Environment
   ...
   As this research is designed to be built upon, having a clear process and
   easily accessible source code and data is important. All the code used in
   this project is made available on the GitHub repository
   https://github.com/iamarcel/thesis. The source for this report is also
   present.

   To maximize ease of use for the authors and future users of this work, Docker
   cite:TODO was used to create a reproducible development environment that can
   easily be set up by anyone. Docker is a system for running lightweight,
   portable containers that make sure all required packages are present in a
   version that is compatible.

   Information on how to use the code is available in the source code
   repository.

*** TODO Mention how you use Pipenv, Docker only for OpenPose
** Getting Pose Data
   ...
   line in its subtitles and mainly consists of the following data points:
...
*** The Video Picker
    ...
    be cut into clips, videos can be processed to extracts only the parts that
    ...
    ~youtube-dl~ cite:TODO which can download the video and embed its
    automatically generated subtitles, it is first examined by the scene
    detection algorithm in ~ffprobe~ cite:TODO. Usually, a person is similarly
    framed throughout a single shot so Video Picker can run semi-automatically
    when a suitable shot is chosen.
    ...
    found a suitable scene, he can point at the person of interest with the
    ...
    Every clip is saved in a JSONL-format cite:TODO (where every line in the
    file is a JSON-formatted object; this is much faster than reading and
    parsing an entire JSON object at once) and the image frames are extracted
    and saved with ~ffmpeg~ cite:TODO automatically by the application.

    The user can also explicitly pick a single clip or stop extracting when the
    shot has changed but the scene detection algorithm hadn't detected that
    change. This happens, for example, when there is a fade between shots.

    In the back end, the video picker is a Python application using the GTK+
    cite:TODO and GStreamer cite:TODO frameworks for building the GUI and
    playing back the video respectively.

**** TODO Figure: screenshot
*** Detecting 2D Poses with OpenPose
    :PROPERTIES:
    :Effort:   0:30
    :END:

    The authors of OpenPose included a sample application that, once compiled,
    can read a directory of images and write the poses in each image to a JSON
    file. This program is run on the output directory of the video picker
    application and afterwards, the pose data from OpenPose is added to the
    database of clips.
    ...
    model OpenPose used cite:TODO. This list is specified for each person
    detected in the image. If, in a clip, one or more joint positions have a
    very small confidence, that clip is not used further.

    There is no stability in the detected people, i.e., people can disappear or
    appear over time and the order in which they are specified can change
    throughout frames. This is why the user must specify the center of the
    target person while selecting video clips in the video picker.

**** TODO Figure: sanity check - example of OpenPose detection
...
*** Lifting Poses to 3D
    ...
    The /3D Pose Baseline/ project had its source code and trained model
    available online so this was used as a starting point. Some modifications
    had to be made in order to use it in this pipeline, though.

    First, the pose data format that 3D Pose Baseline expects is different from
    the one OpenPose outputs. They use the Human3.6M and COCO format,
    respectively. The Human3.6M pose model has its joints ordered differently,
    does not have the eye and ear joints but does define hip, top-of-head and
    spine (at chest height) joints. While the ideal solution would be to
    re-train the 3D Pose Baseline model using 2D data from OpenPose, that would
    require processing their entire training set with OpenPose and then training
    it, which would take too much time. Instead, the following conversion was
    used:
    ...
    - Spine (chest height) :: 1/4 of the distance between Thorax and Hip below
         the Thorax.

    Second, while the authors' code allowed running the training and validation
    steps, there was no code present to run the inference step, i.e., predicting
    3D poses for new 2D detections. Additionally, a smoothing step was added to
    the 2D predictions (OpenPose processes these frame-by-frame) before feeding
    it into this model.

    The results after these two steps were not usable yet to use in a new model
    to predict these gestures, though. First, a missing point in the 2D
    detection had large effects on the results in 3D. In cases where there were
    only joints detected of the upper body, the 3D detection became useless.
    Second, since the data is captured from multiple people, their size and body
    shape differed. Finally, the people in the 3D space were oriented in
    different directions.

**** TODO Figure: sanity check - 3D skeletons

**** TODO Figure: sanity check - video > 2D detection > 3D skeleton (+ subtitle)
...
*** Cleaning and Normalizing the Data
    ...
    - The cleaning step :: throws away corrupt poses and attempts to correct
         small errors.
    - The normalization step :: formats the poses so they are independent of
    ...
    *Cleaning.*
    ...
    - Point error: in a single or a few frames, one or more joints were not
      detected in 2D and had erroneous positions in the 3D output.
    - Clip error: not enough points were detected in 2D, resulting in an
      unusable 3D skeleton.
    - Leaning: a person appeared to be leaning forward while it should not.
    ...
    4 joints had to be corrected this way, the clip is considered low quality
    ...
    *Normalizing.*
    ...
    Since the end goal was to play back gestures on a NAO robot, the choice was
    ...
    autoref:tab-pose-to-angle shows the details of how each angle was
    calculated. Note that when axes are specified, this is in the reference
    coordinate system of the poses returned from 3D Pose Baseline, not the
    coordinate system used in the NAOqi software.

    #+NAME: tab-pose-to-angle
    #+CAPTION: Details of joint position to angle conversion
    | Angle name     | Method                                                         |
    |----------------+----------------------------------------------------------------|
    ...
    Note that these are only the angles for the upper body. The other joints
    and angles are ignored because the upper body is more important and to
    avoid possible issues with the robot getting out of balance.

**** TODO Figure: leaning
     :PROPERTIES:
     :Effort:   0:30
     :END:
**** TODO Figure: NAO skeleton and angles
     :PROPERTIES:
     :Effort:   0:30
     :END:
**** TODO Figure: Vectors used in pose, for directions
     :PROPERTIES:
     :Effort:   1:00
     :END:
**** TODO Figure: Difference in axes
     :PROPERTIES:
     :Effort:   0:15
     :END:
** Finding Motion Primitives
   ...
   Generating gestures can be interpreted as a classification problem if the
   ...
   could classify a sentence under a motion primitive and then concatenate
   these motion primitives into a coherent, continuous whole. This approach
   ...
   natural motion. The second question needs deeper investigation and just
   trying it.

   
*** Time series clustering
    Let us have a closer look at the structure of a clustering algorithm. In
    essence, a clustering algorithm groups "similar-looking" data points, where
    the data points /within/ a group are very similar and data points /across/
    groups differ as much as possible. Additionally the algorithm might produce
    a "prototype" data point for each group. Thus, the primary element of a
    clustering algorithm is its metric for similarity or distance: elements
    within a group have a distance to each other that is as small as possible,
    while their distance to other groups is as large as possible. The
    "prototype" of a group can, for example, be defined as the median of the
    data points in that group.

    In time series clustering, the most important distance measure is the
    Dynamic Time Warping (DTW) metric. This is similar to the Euclidean distance
    but allows time to stretch and compress at will, such as to get the closest
    match between two sequences as possible.

    The ~dtwclust~ R package allows experimenting with different distance
    metrics. Those that support sequences of varying lengths are:

    - Dynamic Time Warping (DTW) distance :: the shortest path through the
         matrix of pairwise distances between the points of two sequences, from
         the top left to the bottom right point.
    - Global Alignment Kernel distance :: a generalization of the DTW distance
         that can use any distance metric as opposed to only the Euclidean
         distance, in this case the Triangular GAK which uses 

**** TODO Explain metrics in dtwclust
     :PROPERTIES:
     :Effort:   0:30
     :END:
...
**** TODO Talk about DTAK or problems with DTW?

    The DTW metric defines a distance between two clips but we are interested in
    /subsequences/ of clips that occur in multiple clips.
