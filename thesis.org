...
*** The Decoder Network
    ...
    \paragraph{Sequence decoder} The other decoder directly generates gestures,
    sequences of poses. The challenges when implementing this decoder are
    ...
    \begin{align*}
      P: x \mapsto &\sum_{t=0}^T\sum_{i=0}^{n-1} \Big[ y_i^{(t)}(x) - \hat{y}_i^{(t)}(x) \Big]^2 \\
      M: x \mapsto &\sum_{t=0}^T\sum_{i=0}^{n-1} \Big[ \big(y_i^{(t)}(x) - y_i^{(t-1)}(x)\big) - 
                   \big(\hat{y}_i^{(t)}(x) - \hat{y}_i^{(t-1)}(x)\big) \Big]^2.
    \end{align*}
    
    Here, we defined $y: x \mapsto y(x)$ as the function that maps the input to
    ...
    #+BEGIN_SRC latex :exports results :results output
      \begin{figure}
        ...
        \begin{tikzpicture}[->, thick, x=\smallgridsize, y=\smallgridsize, font=\headingfont, center coordinate=(cell)]
          ...
          \draw[->]
            (cell.160)++(-2, 0) node(state) [left] {State}
          ...
          \draw[->]
            (cell.20)
            -- node(newstatejoin)[pos=0]{}
              ++(2, 0) node(newstate) [right] {State};
          ...
          \draw 
            (state.east)++(0, 1) 
            node (thoughtvector) [variable block, left, align=center] {Initial State\\{\small(Thought Vector)}}
            .. controls +(1, 0) and (statejoincontrol) .. (statejoin.center);
          ...
          \draw[looseness=3] (newstatejoin.center) to[out=0, in=0] ([shift=(up:1)] cell.north) to[out=180, in=180] (statejoin.center);
...
*** Classification-based vs. \glsentrytext{rnn}
    :PROPERTIES:
    :Effort:   2:00
    :END:
*** Hyperparameters
